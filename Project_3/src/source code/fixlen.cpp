...
// see WriteHeader for header record structure
{
	...
	result = IOBuffer::ReadHeader (stream,NULL);
...
//	record size		2 bytes
{
	...
	result = IOBuffer::WriteHeader (stream,NULL);
	if (!result) return -1;
...
int FixedLengthBuffer::Delete(ostream&, int, int) {
	return 0;
}
unsigned short FixedLengthBuffer::Get_Buffer_Length(istream&, int) {
	return 0;
}
int FixedLengthBuffer::Get_Next_Addr(istream&, int) {
	return 0;
}
int FixedLengthBuffer::Update(ostream&, int recaddr, unsigned short length) {
	return 0;
}
int FixedLengthBuffer::update_Deleted(ostream&, int recaddr, int next) {
	return 0;
}
unsigned short FixedLengthBuffer::Get_Packed_Length() {
	return 0;
}