...
{  public:
	...
	//KHJ
	virtual int Delete(ostream&, int ,int)=0;
	virtual unsigned short Get_Buffer_Length(istream &,int)=0;
	virtual int Get_Next_Addr(istream&, int) = 0;
	virtual int Update(ostream &, int recaddr, unsigned short length) = 0;
	virtual int update_Deleted(ostream&, int recaddr, int next) = 0;
	virtual unsigned short Get_Packed_Length() = 0;
	//KHJ_END
	...
	virtual int ReadHeader (istream &, int*); // write a buffer to the stream
	virtual int WriteHeader (ostream &, int ) const; // write a buffer to the stream
