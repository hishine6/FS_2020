...
int DelimFieldBuffer :: ReadHeader (istream & stream, int* delete_addr)
// read header: contains delimeter character
{
	char ch;
	int result;
	result = VariableLengthBuffer::ReadHeader (stream, delete_addr);
...
int DelimFieldBuffer :: WriteHeader (ostream & stream, int delete_addr) const
// write header into file: contains the delimeter character
{
	if (!Initialized) return FALSE;
	int result;
	result = VariableLengthBuffer::WriteHeader (stream,delete_addr);
	if (!result) return FALSE;
	stream . put (Delim);
